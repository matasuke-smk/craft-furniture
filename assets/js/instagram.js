...
function createInstagramPost(post, index) {
    ...
    item.setAttribute('aria-label', `Instagram投稿 ${index + 1}: ${truncateText(post.caption, 50)}`);
    
    // Create image element
    const img = document.createElement('img');
    img.src = 'data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iNDAwIiBoZWlnaHQ9IjQwMCIgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwMDAvc3ZnIj48cmVjdCB3aWR0aD0iMTAwJSIgaGVpZ2h0PSIxMDAlIiBmaWxsPSIjZjhmOGY4Ii8+PC9zdmc+'; // Placeholder
    img.dataset.src = post.media_url;
    img.alt = `Instagram投稿: ${truncateText(post.caption, 100)}`;
    img.loading = 'lazy';
    
    // Create overlay
    const overlay = document.createElement('div');
    overlay.className = 'instagram__overlay';
    overlay.innerHTML = `
    ...
    const handleClick = (event) => {
        if (event.type === 'click' || (event.type === 'keydown' && (event.key === 'Enter' || event.key === ' '))) {
            ...
            // Track click
            if (typeof trackEvent === 'function') {
                trackEvent('Instagram', 'Post Click', 'External Link', index + 1);
            }
        }
    };
