...
class NewsPagination {
    constructor() {
        this.currentPage = 1;
        this.itemsPerPage = 9;
        this.totalItems = document.querySelectorAll('.news-card').length;
...
class NewsSearch {
    ...
    performSearch(query) {
        ...
        this.newsCards.forEach(card => {
            const title = card.querySelector('.news-card__title a').textContent.toLowerCase();
...
class NewsAnalytics {
    ...
    trackNewsCardClicks() {
        document.querySelectorAll('.news-card__title a').forEach(link => {
            link.addEventListener('click', (e) => {
